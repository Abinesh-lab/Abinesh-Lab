https://github-readme-activity-graph.vercel.app




🛠️ Tech Stack (Skill Icons)


[![My Skills](https://skillicons.dev/icons?i=js,html,css,java,nodejs,react,postman,git,figma,mysql)](https://skillicons.dev)


# Hi 👋 I'm Abinesh R

### Java Full Stack Developer

🎓 B.Tech Information Technology

📍 Coimbatore

💻 Passionate about Full Stack Development and Java

🌱 Currently learning Spring Boot 

🔥Streak.....


[![GitHub Streak](https://streak-stats.demolab.com?user=Abinesh-lab&theme=github-dark)](https://git.io/streak-stats)


## 🚀 Featured Projects

### 🤖 AI Mock Interview Platform
- AI-powered interview preparation platform built with React.js, Node.js, Express.js, MongoDB, and OpenAI API.
- Features JWT authentication, AI-generated questions, analytics dashboard, and performance tracking.

### 🛒 E-Commerce Website
- Full-stack e-commerce application with authentication, product catalog, shopping cart, and order management.

### 🧪 Tichi QA Manual Testing
- Designed and executed 50+ manual test cases covering Login, Signup, Google Authentication, and validation scenarios.
- Reported defects and maintained QA documentation using GitHub.
